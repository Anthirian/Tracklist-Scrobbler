...
class Parser(object):
    ...
    def strip_leading_digits(self, text):
        """
        ...
        artist = text[match.end() if match else 0:].strip()
        return artist
    ...
    def find_featured_artists(self, artist):
        ...
        # If the artist contains a featuredArtist artist we check which term matched our regex, add that term as
        # featuredArtist artist and remove it from the artist itself
        featuredArtist = ""
        if match:
    ...
    def remove_special_track_info(self, line):
        """
        Remove special types indications of a track. Examples of this are
        - FUTURE FAVORITE:
        - ASOT RADIO CLASSIC:
        - [Global Selection]
        - [World Premiere]
        :rtype : str
        :param line: the line from which to remove special track info
        """
    ...
    def parse_tracklist(self, tracks, podcast, hours_ago):
        """Parse a tracklist into artist, title, album, remix, label, etc.
