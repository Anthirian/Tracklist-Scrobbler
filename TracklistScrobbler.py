...
class Interface(Frame):
    ...
    def create_supported_podcasts(self):
        ...
        for name in supportedPodcasts:
            Radiobutton(self.options, text=name, value=name, takefocus=False, variable=self.podcast).grid(sticky=N + W,
                                                                                                          padx=5)

        self.just_listened = BooleanVar()
        Checkbutton(self.options, text="I just finished listening to this podcast", variable=self.just_listened,
                    onvalue=True, offvalue=False).grid(sticky=N + W, padx=5, pady=25)
    ...
    def create_buttons_toolbar(self):
        self.scrobbleButton = Button(self, text="Scrobble", command=self.scrobble, width=30,
                                     state=NORMAL if self.parsed else DISABLED)
    ...
    def parse(self):
        ...
        if not contents:
            self.notify(
                "You have not entered a tracklist. Please provide a tracklist before pressing the Scrobble button.",
                "red")
        else:
            self.clear_notifications()
            if not self.just_listened.get():
                self.time_offset = tkSimpleDialog.askinteger("Please specify a time offset",
                                                             "How long ago did you listen to this podcast (in hours)? Enter 0 for 'just now.'",
                                                             initialvalue="0")
            else:
                self.time_offset = 0
            ...
            if results:
                ...
                self.notify(
                    "The tracklist you provided could not be parsed into valid tracks. Please correct the tracklist and try again.",
                    "red")
    ...
    def scrobble(self):
        ...
        if self.parsed:
            user = self.get_user()
            pw = self.get_pass()
            self.lastfmdata = self.p.parse_user_modifications(self.get_container_contents_filtered(), self.podcast,
                                                              self.time_offset)
            if user and pw:
                try:
                    self.ts.login(user, pw)
                except WSError, wse:
                    tkMessageBox.showerror("Authentication failed",
                                           "Something went wrong during authentication. Last.fm responded:\n\"%s\"" % wse.details)
                except MalformedResponseError, mre:
                    tkMessageBox.showerror("Malformed Response",
                                           "Something is wrong with last.fm:\n\"%s\"\nThis could indicate that the website is currently having problems." % mre)
                ...
                tkMessageBox.showerror("Authentication error",
                                       "One of the login fields is empty. Please fix it before continuing.")
        else:
            self.notify(
                "This tracklist has to be parsed before it can be scrobbled. Please press 'Parse' and then try scrobbling again.",
                "red")
